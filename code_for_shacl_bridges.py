...
# Function to generate variable names for unique entities
...
# Function to generate the SPARQL query text from the DataFrame
def sparql_query_text_from_dataframe(df):
    ...
    unique_from = df['From'].dropna().unique()
    unique_to = df['To'].dropna().unique()
    unique_target = df['Target'].dropna().unique()

    # Generate dynamic variable names for unique entities in 'To', 'From', and 'Target'
    variable_mapping = generate_variable_names(pd.concat([df['From'], df['To'], df['Target']]).dropna().unique())

    # Lists to store the 'some' declarations and the relation statements
    construct_some_statements = []
    construct_relation_statements = []
    where_statements = []

    # Generate the CONSTRUCT 'some' statements and relation statements
    for _, row in df.iterrows():
        to_value = row['To']
        relation_value = row['Relation']
        target_value = row['Target']

        # Add a declaration for each unique 'To' term in the CONSTRUCT block
        construct_some_statements.append(f"{variable_mapping[to_value]} some {to_value}.")

        # Only add if the Relation and Target exist (i.e., not '-')
        if pd.notna(relation_value) and relation_value != '-' and pd.notna(target_value) and target_value != '-':
            construct_relation_statements.append(
                f"{variable_mapping[to_value]} {relation_value} {variable_mapping[target_value]}.")

    # Remove duplicates in the 'some' and relation statements
    construct_some_statements = list(dict.fromkeys(construct_some_statements))
    construct_relation_statements = list(dict.fromkeys(construct_relation_statements))

    # Generate the WHERE statements (all unique From entities)
    for from_value in unique_from:
        where_statements.append(f"{variable_mapping[from_value]} some {from_value}.")

    # Remove duplicates in WHERE block and keep the structure intact
    where_statements = list(dict.fromkeys(where_statements))

    # Construct the final output
    construct_some_block = "\n\t".join(construct_some_statements)
    construct_relation_block = "\n\t".join(construct_relation_statements)
...
# Print the longest path and its length
print("Longest path:", longest_path_i1)
print("Length of longest path:", len(longest_path_i1))
...
print(classes_df[['class_label', 'class_curie']])
...
# Function to build mermaid chart entries for common, SV-specific, and SB-specific entries
...
print(shape_validation_df_common)

# Create the DataFrame from the given table
# data = {
#     'subject_label': ['Process', 'ProcessStep', 'Input'],
#     'predicate_label': ['hasPart', 'hasInput', 'hasModifier'],
#     'object_label': ['ProcessStep', 'Input', 'InputSettings']
# }
#
# df = pd.DataFrame(data)
#
# # Create a directed graph (since the edges can be traversed in both ways, we will treat it as undirected)
# G_1 = nx.Graph()
#
# # Add edges to the graph from the DataFrame
# for index, row in shape_validation_df_common.iterrows():
#     # Add the intended direction with a weight of 1
#     G_1.add_edge(row['subject_label'], row['object_label'], weight=1)
#     # Add the inverse direction with a weight of 2
#     G_1.add_edge(row['object_label'], row['subject_label'], weight=2)
#
# # Compute the closeness centrality for each node
# closeness_centrality = nx.closeness_centrality(G_1, distance='weight')
#
# G_2 = nx.DiGraph()
#
# # Add edges to the graph from the DataFrame
# for index, row in shape_validation_df_common.iterrows():
#     # Add the intended direction with a weight of 1
#     G_2.add_edge(row['subject_label'], row['object_label'])
#
# # Find the node with the highest closeness centrality
# max_centrality_value = max(closeness_centrality.values())
# candidates = [node for node, centrality in closeness_centrality.items() if centrality == max_centrality_value]
#
# # If more than one node has the same closeness centrality, we select the one with the most outgoing edges
# print(G_2.out_degree)
# if len(candidates) > 1:
#     # Count outgoing edges (out-degree) for each candidate
#     candidate_out_degrees = {node: G_2.out_degree(node) for node in candidates}
#     highest_centrality_node = max(candidate_out_degrees, key=candidate_out_degrees.get)
# else:
#     # If there's only one node with the highest centrality, select that node
#     highest_centrality_node = candidates[0]
#
# # Output the node with the highest centrality and its centrality value
# highest_centrality_value = closeness_centrality[highest_centrality_node]
# print(f"Node with the highest closeness centrality: {highest_centrality_node}")
# print(f"Closeness centrality value: {highest_centrality_value}")
...
with open('./shacl_validation.ttl', 'w') as f:
    f.write(final_shacl)
...
generated_text = sparql_query_text_from_dataframe(shape_bridge_df)
...
# Extract prefixes from the SPARQL query
prefixes = extract_prefixes_from_sparql(generated_text)

# Generate the SHACL prefix block
shacl_prefix_block = generate_shacl_prefix_block(prefixes)

# Generate the full text for the SPARQL-based validation
shacl_rule = f"""
...
# Output the generated SHACL rule with prefixes
print(shacl_rule)
